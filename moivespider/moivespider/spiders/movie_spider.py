...
class MovieSpiderSpider(scrapy.Spider):
    ...
    def parse_movie(self, response):
        for sel in response.xpath('//div[@class="co_content8"]//a[re:test(@href,"/html/gndy/.+/\d+.html")]/text()'):
            title = sel.extract().encode('utf-8')
            begin = title.index("《")
            end = title.index("》")
            yield scrapy.Request('https://movie.douban.com/j/subject_suggest?q=' + title[begin + 3:end], callback = self.parse_douban_url, dont_filter = True,
                                 headers = {"User-Agent": "Mozilla/5.0"}, meta = {"name": title[begin + 3:end]})

    # 解析电影详情页地址
    def parse_douban_url(self, response):
        res = json.loads(response.body)
        url = res[0]['url']
        yield scrapy.Request(url, callback = self.parse_rating_num, headers = {"User-Agent": "Mozilla/5.0"}, dont_filter = True, meta = {"name": response.meta["name"]})

    # 解析打分
    def parse_rating_num(self, response):
        rate_num = float(response.xpath('//strong[contains(@class,"rating_num")]/text()').extract()[0])
        if rate_num > 8.0:
            print response.meta['name'], rate_num
